...
protected:
    ...
    // ============================================================================
    // BOX SELECTION (NO DUPLICATES!)
    // ============================================================================
    
    /** Is user currently dragging to select? */
    bool bIsBoxSelecting = false;
    
    /** Start position of box selection (screen space) */
    FVector2D BoxSelectionStart;
    
    /** Current mouse position during box selection */
    FVector2D BoxSelectionEnd;
    
    /** Visual box selection widget */
    UPROPERTY()
    UUserWidget* BoxSelectionWidget;
    
    /** Class for box selection widget */
    UPROPERTY(EditDefaultsOnly, Category = "Selection")
    TSubclassOf<UUserWidget> BoxSelectionWidgetClass;
    
    /** Start box selection */
    void StartBoxSelection(const FVector2D& ScreenPosition);
    
    /** Update box selection */
    void UpdateBoxSelection(const FVector2D& ScreenPosition);
    
    /** End box selection and select all slots in rectangle */
    void EndBoxSelection();
    
    /** Get all slots within the selection rectangle */
    TArray<UInventorySlotWidget*> GetSlotsInRectangle(const FVector2D& TopLeft, const FVector2D& BottomRight);
